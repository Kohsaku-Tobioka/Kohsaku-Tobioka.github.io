...
def make_filename(year, month, day):
    return f"{year}-{MONTHS[month]}{day.zfill(2)}-seminar.md"

def make_markdown(row):
    title = row["Title"]
    venue = row["Seminartype"]
...
location: "{location}"
---
**Seminar**: *{venue}*, {location}. *{row["Para1"]}*
...
input_csv = os.path.join(script_dir, 'Seminar_Talk_list.csv')
